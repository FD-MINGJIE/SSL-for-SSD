...
from ssl_ssd import build_ssd_con

from VAT08 import VAT # change config
...
def train():
    if args.dataset == 'COCO':
        ...
        from data.config_vat import voc300_vat
        cfg = voc300_vat
    ...
    while(finish_flag):
        vat = VAT()
        ssd_net = build_ssd_con('train', cfg['min_dim'], cfg['num_classes'], flip=True)
        net = ssd_net
        ...
        for iteration in range(args.start_iter, cfg['max_iter'] + 50000 ):    # 17W iter
            ...
            try:
                images, targets, semis = next(batch_iterator)	# not resume

            except StopIteration:	# resume
                supervised_flag = 0
                supervised_dataset = VOCDetection_con(root=args.dataset_root,
                                                         transform=SSDAugmentation(cfg['min_dim'],
                                                                                      MEANS))#, shuffle_flag=data_shuffle)
            ...
            # vat
            x_hat = vat(images, targets, semis, net, iteration).detach()
            #images = x_hat
               
            # forward
            t0 = time.time()
            out, conf, conf_flip, loc, loc_flip = net(images, x_hat, isflip=True)    # [batch,3,300,300]

            sup_image_binary_index = np.zeros([len(semis),1])

            for super_image in range(len(semis)):     #[0,32]
                if(int(semis[super_image])==1):
                    sup_image_binary_index[super_image] = 1	# voc2007
                else:
                    sup_image_binary_index[super_image] = 0 # voc2012
                
                if(int(semis[len(semis)-1-super_image])==0):
                    del targets[len(semis)-1-super_image]	# unlabeled voc2012
            ...
            if iteration != 0 and (iteration+1) % 10000 == 0:
                ...
                torch.save(ssd_net.state_dict(), 'weights/ssd300_VAT08_single_' +
        ...
        if((iteration +1) ==cfg['max_iter'] + 50000):    
            finish_flag = False


def rampweight(iteration):
    ramp_up_end = 32000
    ramp_down_start = 100000 #100000

    if(iteration<ramp_up_end):
        ramp_weight = math.exp(-5 * math.pow((1 - iteration / ramp_up_end),2))
    elif(iteration>ramp_down_start):
        ramp_weight = math.exp(-12.5 * math.pow((1 - (120000 - iteration) / 20000),2)) # math.pow((1 - (120000 - iteration) / 20000),2)) 
    else:
        ramp_weight = 1 
